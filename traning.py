...
def load_data():
    data = np.empty((110,1,28,28),dtype="float32")   #empty与ones差不多原理，但是数值随机，类型随后面设定
    label = np.empty((110,),dtype="uint8")

    imgs = os.listdir("ch")   #打开文件夹里的目录
    num = len(imgs)
    for i in range(num):
        img = Image.open("ch/"+imgs[i])   #用图像处理函数Image处理图像
        arr = np.asarray(img,dtype="float32")  #将img数据转化为数组形式
        data[i,:,:,:] = arr   #将每个三维数组赋给data
        label[i] = int(imgs[i].split('.')[1])   #取该图像的数值属性作为标签
    return data,label
...
#<code class="hljs vala has-numbering"><span class="hljs-preprocessor">建立CNN模型</span></code>
